...
The approach is simple, Percieve, Reason and Act. This integrates five components: (1) Rotary Positional Embeddings (RoPE) leveraging Euler's formula for infinite-horizon generalization, (2) Tree--Transformer blocks with functional gated leaves (a soft hierarchical mixture of experts), (3) Bayesian linear layers for epistemic uncertainty estimation, (4) curiosity-driven exploration based on routing entropy and Bayesian KL divergence and (5) hindsight experience replay for return relabeling. Experiments on continuous-control tasks suggest that BAA can match strong baselines while producing human-readable decision paths that support debugging and safety interventions via \emph{surgical pruning}.
\end{abstract}
